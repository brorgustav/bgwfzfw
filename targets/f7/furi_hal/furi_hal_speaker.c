...
// Internal declaration for custom speaker pin (not in public API)
...
void furi_hal_speaker_release(void) {
  ...
  // Runtime speaker pin selection
  if (bgwx_get_speaker_mode()) {
    furi_hal_gpio_init(&bgw_gpio_speaker, GpioModeAnalog, GpioPullDown,
                       GpioSpeedLow);
  } else {
    furi_hal_gpio_init(&gpio_speaker, GpioModeAnalog, GpioPullDown,
                       GpioSpeedLow);
  }
...
static inline uint32_t furi_hal_speaker_calculate_compare(float volume) {
  ...
  uint32_t clip_value =
      volume * LL_TIM_GetAutoReload(FURI_HAL_SPEAKER_TIMER) / 2;
...
void furi_hal_speaker_start(float frequency, float volume) {
  ...
  LL_TIM_OC_Init(FURI_HAL_SPEAKER_TIMER, FURI_HAL_SPEAKER_CHANNEL,
                 &TIM_OC_InitStruct);
...
void furi_hal_speaker_set_volume(float volume) {
  ...
  LL_TIM_OC_SetCompareCH1(FURI_HAL_SPEAKER_TIMER,
                          furi_hal_speaker_calculate_compare(volume));
#else
